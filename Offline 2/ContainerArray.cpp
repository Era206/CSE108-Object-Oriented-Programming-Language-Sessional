...
public:
    ...
    void setAllocatedSize(int sz){
        ...
        // allocate memory dynamically as required
        if(arrayOfContainers!=NULL) delete [] arrayOfContainers;
         allocatedSize=sz;
        arrayOfContainers = new Container[allocatedSize];
    ...
    void setItemAt(Container ct, int index)
    {
        if (index >= allocatedSize){
            cout << "Exception: Container Array index out of bound";
            exit(0);
        }
        if(ct.getStoredType()==INTEGER)
        {
            arrayOfContainers[index].setItem(*((int*)ct.getItem()));
        }
        else if(ct.getStoredType()==INT_ARRAY)
        {
            arrayOfContainers[index].setItem((int*)ct.getItem(), ct.getFirstDim());
        }
        else if(ct.getStoredType()==INT_MATRIX)
        {
            arrayOfContainers[index].setItem((int**)ct.getItem(), ct.getFirstDim(), ct.getSecondDim());
        }
    }
